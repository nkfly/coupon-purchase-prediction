...
def process_dis_time(disfrom, disend):
    disfrom = datetime.datetime.strptime(disfrom, '%Y-%m-%d %H:%M:%S')
    disend = datetime.datetime.strptime(disend, '%Y-%m-%d %H:%M:%S')
    days = (disend-disfrom).days

    daysDistribution = [0] * 7

    for i in range(days):
        daysDistribution[(disfrom + datetime.timedelta(days=(i%7))).weekday()] += 1
...
def coupon_row_to_vector(row, genre_dict, large_area_dict, ken_dict, small_area_dict):
    ...
    vector.extend(process_dis_time(row['DISPFROM'], row['DISPEND']))
...
def compose_train_data(coupon_detail_train, user_hash_to_vector_dict, train_coupon_hash_to_vector_dict):
    ...
    with open(coupon_detail_train, 'r') as csvfile:
        ...
        for row in spamreader:
            ...
            if dtime.weekday() not in user_hash_to_day_probability[user_hash]:
                user_hash_to_day_probability[user_hash][dtime.weekday()] = 1
            else:
                user_hash_to_day_probability[user_hash][dtime.weekday()] += 1
            # 2012-03-28 15:06:06
            # user_hash_to_coupon_list[user_hash].append(coupon_hash)
    ...
    for user_hash in user_hash_to_day_probability:
        sumOfBuy = 0
...
def get_user_buy_and_view(coupon_visit_train):
    user_buy_and_view = {}
    with open(coupon_visit_train, 'r') as csvfile:
        spamreader = csv.DictReader(csvfile)
        for row in spamreader:
            user_hash = row['USER_ID_hash']
            coupon_hash = row['VIEW_COUPON_ID_hash']
...
def main():
    ...
    user_hash_to_train_coupon_list, user_hash_to_day_probability = compose_train_data(coupon_detail_train, user_hash_to_vector_dict, train_coupon_hash_to_vector_dict)
    ...
    with open(coupon_list_test, 'r') as csvfile:
        ...
        for row in spamreader:
            ...
            for user_hash in user_hash_to_coupon_average_cosine_distance:
                user_hash_to_coupon_average_cosine_distance[user_hash][coupon_hash] = average_cosine_distance(user_hash, coupon_vector, train_coupon_hash_to_vector_dict, user_hash_to_train_coupon_list, user_buy_and_view, user_hash_to_day_probability)
