#Only change the negative sample rate => different pref using different negative sample rate
...
def compose_train_data(coupon_detail_train, user_hash_to_vector_dict, train_coupon_hash_to_vector_dict, user_hash_to_pref, pref_name_dict):
    train_data = []
    train_data_label = []
    positive_sample_num = {}
    ...
    with open(coupon_detail_train, 'r') as csvfile:
        ...
        for row in spamreader:
            ...
            if user_hash_to_pref[user_hash] in positive_sample_num:
                positive_sample_num[user_hash_to_pref[user_hash]] += 1
            else:
                positive_sample_num[user_hash_to_pref[user_hash]] = 1
    ...
    #total train coupon number : 19413
    negative_sample_num = {}
    negative_sampling_rate = {}
    for pref in pref_name_dict:
        if pref not in negative_sample_num:
            negative_sample_num[pref] = 0
        for user in user_hash_to_vector_dict:
            if user in train_data_buy and user_hash_to_pref[user] == pref:
                negative_sample_num[pref] += 1 * (19413 - len(train_data_buy[user_hash]))
    for pref in negative_sample_num:
        negative_sampling_rate[pref] = float(positive_sample_num[pref])/negative_sample_num[pref]
    
    with open('negative_sample_num_pref.csv', 'wb') as f:
        w = csv.DictWriter(f, negative_sample_num.keys())
        w.writeheader()
        w.writerow(negative_sample_num)
    with open('negative_sampling_rate_pref.csv', 'wb') as f:
        w = csv.DictWriter(f, negative_sampling_rate.keys())
        w.writeheader()
        w.writerow(negative_sampling_rate)

    for user_hash in user_hash_to_vector_dict:
        for train_coupon_hash in train_coupon_hash_to_vector_dict:
            if random.random() < negative_sampling_rate[user_hash_to_pref[user_hash]] and user_hash in train_data_buy and train_coupon_hash not in train_data_buy[user_hash]:
                user_vector_and_coupon_vector = list(user_hash_to_vector_dict[user_hash])
...
def main():
    ...
    train_data, train_data_label = compose_train_data(coupon_detail_train, user_hash_to_vector_dict, train_coupon_hash_to_vector_dict, user_hash_to_pref, pref_name_dict)
